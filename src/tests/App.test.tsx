...
describe('App Component', () => {
  ...
  it('debería filtrar por la región Hoenn', async () => {
    const mockFetch = vi.fn()
    globalThis.fetch = mockFetch
    setupMockFetch([{ id: 1, name: 'bulbasaur' }], mockFetch)
    setupMockFetch([{ id: 3, name: 'charizard' }], mockFetch)
    ...
    const opcion = await screen.findByText(/hoenn/i)
    await userEvent.click(opcion)

    expect(await screen.findByText('charizard')).toBeVisible()
    expect(globalThis.fetch).toHaveBeenNthCalledWith(
      3,
      `https://pokeapi.co/api/v2/pokemon?offset=251&limit=386`,
    )
  })
  ...
  it('debería filtrar por la región Unova', async () => {
    const mockFetch = vi.fn()
    globalThis.fetch = mockFetch
    setupMockFetch([{ id: 1, name: 'bulbasaur' }], mockFetch)
    setupMockFetch([{ id: 3, name: 'charizard' }], mockFetch)
    ...
    const opcion = await screen.findByText(/unova/i)
    await userEvent.click(opcion)

    expect(await screen.findByText('charizard')).toBeVisible()
    expect(globalThis.fetch).toHaveBeenNthCalledWith(
      3,
      `https://pokeapi.co/api/v2/pokemon?offset=494&limit=649`,
    )
  })

  it('debería filtrar por la región Kalos', async () => {
    const mockFetch = vi.fn()
    globalThis.fetch = mockFetch
    setupMockFetch([{ id: 1, name: 'bulbasaur' }], mockFetch)
    setupMockFetch([{ id: 3, name: 'charizard' }], mockFetch)
    ...
    const opcion = await screen.findByText(/kalos/i)
    await userEvent.click(opcion)

    expect(await screen.findByText('charizard')).toBeVisible()
    expect(globalThis.fetch).toHaveBeenNthCalledWith(
      3,
      `https://pokeapi.co/api/v2/pokemon?offset=649&limit=721`,
    )
  })
  ...
  it('debería filtrar por la región Galar', async () => {
    const mockFetch = vi.fn()
    globalThis.fetch = mockFetch
    setupMockFetch([{ id: 1, name: 'bulbasaur' }], mockFetch)
    setupMockFetch([{ id: 3, name: 'charizard' }], mockFetch)
    ...
    const opcion = await screen.findByText(/galar/i)
    await userEvent.click(opcion)

    expect(await screen.findByText('charizard')).toBeVisible()
    expect(globalThis.fetch).toHaveBeenNthCalledWith(
      3,
      `https://pokeapi.co/api/v2/pokemon?offset=809&limit=905`,
    )
  })
  ...
  it('debería ordenar las cartas por puntos de ataque', async () => {
    render(<App />)

    const combobox = screen.getByRole('combobox', { name: /sort by/i })
    await userEvent.click(combobox)
    const option = screen.getAllByLabelText('Attack')
    await userEvent.click(option[0])
    const bulbasaur = screen.getByText('bulbasaur')
    const ivysaur = screen.getByText('ivysaur')
  ...
  it('debería ordenar las cartas por puntos de defensa', async () => {
    render(<App />)

    const combobox = screen.getByRole('combobox', { name: /sort by/i })
    await userEvent.click(combobox)
    const option = screen.getAllByLabelText('Defense')
    await userEvent.click(option[0])
    const bulbasaur = screen.getByText('bulbasaur')
    const ivysaur = screen.getByText('ivysaur')
  ...
  it('debería ordenar las cartas por puntos de ataque especial', async () => {
    render(<App />)

    const combobox = screen.getByRole('combobox', { name: /sort by/i })
    await userEvent.click(combobox)
    const option = screen.getAllByLabelText('Special attack')
    await userEvent.click(option[0])
    const bulbasaur = screen.getByText('bulbasaur')
    const ivysaur = screen.getByText('ivysaur')
  ...
  it('debería ordenar las cartas por puntos de velocidad', async () => {
    render(<App />)

    const combobox = screen.getByRole('combobox', { name: /sort by/i })
    await userEvent.click(combobox)
    const option = screen.getAllByLabelText('Speed')
    await userEvent.click(option[0])
    const bulbasaur = screen.getByText('bulbasaur')
    const ivysaur = screen.getByText('ivysaur')
